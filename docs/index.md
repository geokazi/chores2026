...
| 2026-01-25 | [**Confetti Celebrations**](./milestones/20260125_confetti_celebrations.md) | ✅ Complete | Multi-sensory celebrations: confetti (green/gold/multi-color), Web Audio tones, haptic feedback (Android + iOS 18+ hack), weekly finale (5s rain), element animations (CSS pulse/glow/shake); Settings toggle |
...
|------|----------|--------|---------|
| 2026-02-06 | [Admin Page Access Control](./decisions/20260206_admin_page_access_control.md) | ✅ Implemented | Staff email-based access control for admin pages; pattern documentation |
...
- ✅ **Manual Mode Inline Chores**: Create one-time and recurring chores directly in Template Selector UI, view existing chores by type, edit/delete functionality; APIs: `/api/chores/recurring` (GET), `/api/chores/[chore_id]/edit` (POST), `/api/chores/[chore_id]/delete` (POST)
- ✅ **UTC Timezone Fixes**: All date calculations use local date components instead of `toISOString()` to avoid day shifts when local time differs from UTC
